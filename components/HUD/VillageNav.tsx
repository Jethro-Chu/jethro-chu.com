...
export function VillageNav() {
  ...
  const go = (id: string, section: string) => {
    ...
    setLabel(section);
    window.setTimeout(() => gameBus.emit("landmark:enter", { id }), 820);
          ...
          {landmarks.map((l) => (
            <li key={l.id} className="shrink-0">
              <button
                ...
                className="fast-ui label-mono rounded-sm px-2.5 py-1.5 text-[0.72rem] font-medium text-[var(--color-shadow)] hover:bg-[var(--color-pine)] hover:text-[var(--color-on-dark)]"
              >
                {l.section}
              </button>
            </li>
