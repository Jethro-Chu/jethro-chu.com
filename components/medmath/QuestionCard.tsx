    ...
    <div className="w-full rounded-md border border-[var(--color-line)] bg-[var(--color-surface)] shadow-xs">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--color-line)] bg-[var(--color-sand)]/40 px-5 py-3 sm:px-6">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-xs bg-[var(--color-pine)] px-2.5 py-1 font-mono text-[11px] font-semibold uppercase tracking-wider text-white">
            {catMeta?.name ?? question.category}
          </span>
          <span className="rounded-xs border border-[var(--color-line)] bg-[var(--color-surface)] px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-[var(--color-ink-muted)]">
            {question.difficulty.replace("-", " ")}
          </span>
          {question.clinicalContext && (
            <span className="hidden font-mono text-[11px] text-[var(--color-ink-muted)] sm:inline">
              • {question.clinicalContext}
            </span>
          )}
        </div>
      ...
      <div className="p-5 sm:p-6 space-y-5">
        ...
        <p className="font-body text-sm leading-relaxed text-[var(--color-ink)] sm:text-base">
          {question.scenario}
        </p>

        {/* Clinical Medication Order Banner */}
        <div className="rounded-md border border-[var(--color-line)] bg-[var(--color-sand)]/80 p-4">
          <div className="mb-1.5 flex items-center gap-2">
            <span className="flex h-4 w-4 items-center justify-center rounded-full bg-[var(--color-pine)] text-[9px] font-bold text-white">
              Rx
            </span>
          ...
          <div className="font-mono text-sm font-semibold text-[var(--color-ink)] sm:text-base">
            {question.orderText}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {question.availableText && (
            <div className="rounded-sm border border-[var(--color-line)]/70 bg-[var(--color-surface)] p-3.5">
              ...
              <div className="mt-1 font-mono text-xs font-medium text-[var(--color-ink)]">
                {question.availableText}
              </div>
            </div>
          )}
          ...
          {(question.patientWeightKg !== undefined || question.patientWeightLb !== undefined) && (
            <div className="rounded-sm border border-[var(--color-line)]/70 bg-[var(--color-surface)] p-3.5">
              ...
              <div className="mt-1 flex items-baseline gap-2 font-mono text-xs font-semibold text-[var(--color-ink)]">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <div className="relative flex-1 max-w-xs">
                <input
                  ...
                  className="w-full rounded-sm border border-[var(--color-line)] bg-[var(--color-sand)]/30 px-3.5 py-2.5 pr-14 font-mono text-base font-medium text-[var(--color-ink)] focus:border-[var(--color-primary)] focus:outline-hidden disabled:bg-gray-100 disabled:text-gray-500"
                />
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 font-mono text-xs font-semibold text-[var(--color-ink-muted)]">
              ...
              {!gradeResult ? (
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-sm bg-[var(--color-pine)] px-5 py-2.5 font-mono text-xs font-semibold uppercase tracking-wider text-white shadow-xs transition-colors hover:bg-[var(--color-pine)]/90 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  {isSubmitting ? "Checking..." : "Check Answer"}
                </button>
              ) : gradeResult.isCorrect ? (
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5 rounded-sm bg-[var(--color-pine)] px-3 py-2 font-mono text-xs font-semibold text-white">
                    <span>✓</span>
                    <span>Correct</span>
                  </div>
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-sm bg-[var(--color-pine)] px-5 py-2 font-mono text-xs font-semibold uppercase tracking-wider text-white shadow-xs transition-colors hover:bg-[var(--color-pine)]/90"
                  >
                    Next Question →
                  </button>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="flex items-center gap-1.5 rounded-sm border border-red-300 bg-red-50 px-3 py-2 font-mono text-xs font-semibold text-red-700">
                    <span>✗</span>
                  ...
                  <button
                    ...
                    className="rounded-sm border border-[var(--color-line)] bg-[var(--color-surface)] px-3 py-2 font-mono text-xs font-medium text-[var(--color-ink)] transition-colors hover:bg-[var(--color-sand)]"
                  >
                    Try Again
                  </button>
                  {!solutionSteps && (
                    <button
                      ...
                      className="rounded-sm border border-[var(--color-line)] bg-[var(--color-surface)] px-3 py-2 font-mono text-xs font-medium text-[var(--color-ink-muted)] transition-colors hover:text-[var(--color-ink)] hover:bg-[var(--color-sand)]"
                    >
                  ...
                  <button
                    type="button"
                    onClick={onNextQuestion}
                    className="rounded-sm bg-[var(--color-pine)] px-4 py-2 font-mono text-xs font-semibold uppercase tracking-wider text-white transition-colors hover:bg-[var(--color-pine)]/90"
                  >
                    Skip to Next →
                  </button>
                </div>
          ...
          <div className="space-y-4 pt-2">
            <div className="flex items-center max-w-xs relative">
              <input
                ...
                className="w-full rounded-sm border border-[var(--color-line)] bg-[var(--color-sand)]/30 px-3.5 py-2.5 pr-14 font-mono text-base font-medium text-[var(--color-ink)] focus:border-[var(--color-primary)] focus:outline-hidden"
              />
              <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 font-mono text-xs font-semibold text-[var(--color-ink-muted)]">
                {question.expectedUnit}
              </div>
