    ...
    <div className="space-y-12 py-4">
      ...
      <section className="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <div className="rounded-md border border-[var(--color-line)] bg-[var(--color-surface)] p-6 shadow-xs space-y-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-sm bg-[var(--color-sand)] text-base font-bold text-[var(--color-primary)]">
            01
          </div>
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {MEDMATH_CATEGORIES.map((cat) => (
            <div
              key={cat.id}
              className="rounded-md border border-[var(--color-line)] bg-[var(--color-surface)] p-5 shadow-xs space-y-2.5 hover:border-[var(--color-primary)] transition-colors"
            >
              <div className="flex items-center justify-between">
                <span className="text-base font-semibold text-[var(--color-ink)]">
                  {cat.name}
                </span>
                <span className="rounded-xs bg-[var(--color-sand)] px-2.5 py-0.5 text-xs text-[var(--color-ink-muted)] font-medium">
      ...
      <section className="rounded-md border border-[var(--color-line)] bg-[var(--color-surface)] p-6 space-y-2.5">
        ...
        <p className="text-sm sm:text-base leading-relaxed text-[var(--color-ink-muted)]">
          All questions on this platform are calibrated strictly to adult inpatient Medical-Surgical and Critical Care clinical dosing guidelines. Dosing concentrations reflect standard hospital formularies (such as Norepinephrine 8 mg/250 mL, Heparin 25,000 units/500 mL, and Regular Insulin 100 units/100 mL). Patient weights are bounded within realistic adult metrics (50 to 120 kg / 110 to 264 lb).
        </p>
      </section>
    </div>
  );
